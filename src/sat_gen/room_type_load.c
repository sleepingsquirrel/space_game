...
room_type *load_room_types(const char *filename)
{
    ...
    while (fread(&buffer, sizeof(char), 1, file))
    {
        ...
        else
        {
            ...
            for (i = 0; i < j - 2; i++)
            {
                current->descriptions[i] = malloc(strlen(words[i + 4])  + 1);
                memset(current->descriptions[i], 0, strlen(words[i + 4]) + 1);
                for (int c = 0; c < strlen(words[i + 4]) + 1; c++)
                {
                   current->descriptions[i][c] = words[i + 4][c + 1];
                }
            }
